...
export function LogoMarquee({
  ...
  return (
    ...
    >
      ...
      >
        <div ref={trackRef} style={{ gap, willChange: "transform" }} className="logo-marquee-track">
          {Array.from({ length: groups }, (_, copy) => (
            ...
            >
              {items.map((item) => (
                <li key={item.id} className="logo-marquee-item">
                  {copy !== live ? (
                    <span className="logo-marquee-face">{item.mark ?? item.label}</span>
                  ) : item.href ? (
                    <a href={item.href} className="logo-marquee-face logo-marquee-hit">
                      {face(item)}
                    </a>
                  ) : onSelect ? (
                    <button
                      ...
                      className="logo-marquee-face logo-marquee-hit"
                    >
                      {face(item)}
                    </button>
                  ) : (
                    <span className="logo-marquee-face">{face(item)}</span>
                  )}
                </li>
              ))}
            </ul>
      ...
      <div aria-hidden className="logo-marquee-fade logo-marquee-fade-l" />
      <div aria-hidden className="logo-marquee-fade logo-marquee-fade-r" />
    </section>
  );
}
